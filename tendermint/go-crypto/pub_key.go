...
import (
	...
	"github.com/bcbchain/bclib/tendermint/ed25519/extra25519"
...
func (pubKey PubKeyEd25519) Address(chainID string) Address {
	...
	if strings.Contains(chainID, "[") {
		start := strings.Index(chainID, "[")
		end := strings.Index(chainID, "]")
		mainChainID = chainID[:start]
		chainName = chainID[start+1 : end]
	}
