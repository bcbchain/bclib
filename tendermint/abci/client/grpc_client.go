...
func (cli *grpcClient) GetGenesisAsync() *ReqRes {
	req := types.ToRequestGetGenesis()
	res, err := cli.client.GetGenesis(context.Background(), req.GetGenesis(), grpc.FailFast(true))
	if err != nil {
		cli.StopForError(err)
	}
	return cli.finishAsyncCall(req, &types.Response{&types.Response_GetGenesis{res}})
}

func (cli *grpcClient) RollbackAsync() *ReqRes {
	req := types.ToRequestRollback()
	res, err := cli.client.Rollback(context.Background(), req.GetRollback(), grpc.FailFast(true))
	if err != nil {
		cli.StopForError(err)
	}
	return cli.finishAsyncCall(req, &types.Response{&types.Response_Rollback{res}})
}
...
func (cli *grpcClient) GetGenesisSync() (*types.ResponseGetGenesis, error) {
	reqres := cli.GetGenesisAsync()
	return reqres.Response.GetGenesis(), cli.Error()
}

func (cli *grpcClient) RollbackSync() (*types.ResponseRollback, error) {
	reqres := cli.GetGenesisAsync()
	return reqres.Response.Rollback(), cli.Error()
}
