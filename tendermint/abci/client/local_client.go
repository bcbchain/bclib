...
func (app *localClient) GetGenesisAsync() *ReqRes {
	res := app.Application.GetGenesis()
	return app.callback(
		types.ToRequestGetGenesis(),
		types.ToResponseGetGenesis(res),
	)
}

func (app *localClient) RollbackAsync() *ReqRes {
	res := app.Application.Rollback()
	return app.callback(
		types.ToRequestRollback(),
		types.ToResponseRollback(res),
	)
}
...
func (app *localClient) GetGenesisSync() (*types.ResponseGetGenesis, error) {
	res := app.Application.GetGenesis()
	return &res, nil
}

func (app *localClient) RollbackSync() (*types.ResponseRollback, error) {
	res := app.Application.Rollback()
	return &res, nil
}
